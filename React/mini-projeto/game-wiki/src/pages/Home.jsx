...
const Home = () => {
  ...
  return (
    <div className="flex flex-col items-center bg-zinc-950 min-h-screen py-10">
      ...
      <h1 className="text-2xl md:text-3xl font-bold text-white mb-10 uppercase tracking-tighter">
      ...
      <div className="flex items-center flex-col justify-center w-full">
          ...
          {mode === "pokemon" && (
            <>
              <input
                ...
                className="mb-2 px-4 py-2 bg-zinc-800 text-white font-pixel text-xs rounded-xl border border-zinc-700 focus:outline-none focus:border-zinc-500"
              />
              {notFoundMessage && (
                <p className="mb-4 text-xs font-pixel text-red-300">
                  {notFoundMessage}
                </p>
              )}
            </>
        ...
        {loading ? (
          ...
          <div className="grid w-full grid-cols-[repeat(auto-fill,minmax(250px,4px))] gap-4 justify-center">
            {items.map((item) => (
              <AgentCard
                key={item.id}
                id={item.id}
