# 📁 Treinamento: Base de Programação (Alura)

Repositório dedicado aos estudos da trilha de **Base de Programação**, com foco na organização de exercícios e anotações.

---

## 🏗️ Estrutura da Trilha

### 1. Lógica e Algoritmos ✅
* [x] Pensamento computacional: fundamentos da computação e lógica de programação
* [x] Lógica de programação: mergulhe em programação com JavaScript
* [x] Lógica de programação: explore funções e listas
* [x] Lógica de programação: praticando com desafios

### 2. Orientação a Objetos
* [x] JavaScript: programação Orientada a Objetos
* [x] Object Calisthenics: exercitando a Orientação a Objetos

### 3. Controle de Versão ⏳
* [x] Git e GitHub: compartilhando e colaborando em projetos
* [x] Git e GitHub: dominando controle de versão de código

### 4. Infraestrutura e Ambiente 📅
* [x] Terminal: aprenda comandos para executar tarefas
* [x] Windows Prompt: utilizando o CMD
* [x] Artigo - WSL: executando Linux no Windows
* [x] HTTP: entendendo a web por baixo dos panos
* [x] VSCode: aprendendo dicas e truques
* [x] Docker: criando e gerenciando containers

### 5. Banco de Dados 📅
* [ ] Modelagem de banco de dados relacional: entendendo SQL
* [ ] MySQL: conhecendo a ferramenta
* [ ] MongoDB: conhecendo um banco de dados NoSQL

### 6. IA
* [ ] Engenharia de Prompt: criando prompts eficazes para IA Generativa
* [ ] GitHub Copilot e ChatGPT: impulsionando seu desenvolvimento com IA

---
> **Status Atual:** 4/6 Módulos Concluídos
---
*Organizado por: Gabriel Maiolli de Oliveira*